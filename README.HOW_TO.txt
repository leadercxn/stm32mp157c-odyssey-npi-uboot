...
If you have never configured you git configuration:
    $ git confir_name"
    $ git config --global user.email "your_email@examg --global user.name "youple.com"
...
* To verify if you cross-compil
ation environment are put in place:
    $ set | grep CROSS
...
In the U-Boot source directory (sources/*/u-boot-stm32mp-2020.10.r1-r0),
you have one U-Boot source tarball, the patches and one Makefile:
   - u-boot-stm32mp-2020.10.r1-r0.tar.gz
   - 000*.patch
   - Makefile.sdk
...
Then you must extract the tarball and apply the patch:
    ...
    $> tar xfz u-boot-stm32mp-2020.10.r1-r0.tar.gz
...
4.1 Get STMicroelectronics U-Boot source from GitHub
    ...
    Branch: v2020.10.r1-stm32mp
    Revision: v2020.10.r1-stm32mp-r0

    $ git clone https://github.com/STMicroelectronics/u-boot.git
    $ git checkout -b WORKING v2020.10.r1-stm32mp-r0
...
or
    ...
    $ cd u-boot
    $ git checkout -b WORKING v2020.10.r1
    $ for p in `ls -1 ../*.patch`; do git am $p; done
